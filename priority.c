...
int main()
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        scanf("%d", &P[i].at);
        printf("Enter Priority for Process %d: ", P[i].p);
        scanf("%d", &P[i].pr);
    }
    ...
    // Sort processes by Priority after sorting by arrival time (SJF scheduling)
    for(i = 1; i < n; i++)
    {
        for(j = i+1; j < n; j++)
        {
    ...
    // Table
    printf("Table:\n");
    printf("Process\tAT\tBT\tPR\tST\tFT\tWT\tTAT\n");
    for(i = 0; i < n; i++)
    {
        printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", P[i].p, P[i].at, P[i].bt, P[i].pr, P[i].st, P[i].ft, P[i].wt, P[i].tat);
    }
